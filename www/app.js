...
document.addEventListener(
  ...
  function (e) {
    ...
    // up button
    if (app.position.mode == "quran") navigator.app.exitApp();
    else {
      history.back();
...
if (chrome_app) $("#installation #chrome").hide();
if (android_app) $("#installation").hide();

$("[rel=menu]").show();
if (android_app) $(".quran [rel=menu]").hide();
...
var initApp = function () {
  ...
  if (Backbone.history.getFragment() == "") {
    ...
    if (position.mode == "quran") {
      ...
      else address = "quran/p" + position.quran.page;
      ...
      if (position.tafsir.aya) address += position.tafsir.aya;
      else address += position.tafsir.section + "/i" + position.tafsir.part;
    }
...
$(window).resize(function () {
  $(".twitter-typeahead .tt-dropdown-menu").css(
    "max-height",
    $("#views").height()
  );
});
...
$("#views").swipe({
  tap: function (e) {
    if (e.type != "mouseup") {
      ...
      if (e.target.id == "page") $(e.target).toggleClass("active");
    }
  },
  swipeLeft: function () {
...
var requestUrls = function (urls, index, isStored, storeData, progress) {
  if (index >= urls.length && progress) progress(100);
...
var download_tafsir = function () {
  ...
  urls = _.map(almizan_sections, function (section) {
    return "almizan_" + variables.lang + "/" + section;
  });
...
var download_quran = function (origin) {
  if (!localStorage.Quran || localStorage.Quran.split(",").length < 6230) {
    request = $.when($.get(origin + "all"), $.getJSON(origin + "fa")).done(
      function (all, fa) {
...
// main
if (!android_app || localStorage.Quran) initApp();
else {
