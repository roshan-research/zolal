...
var QuranView = Backbone.View.extend({
	...
	render: function() {
		...
		var front = pages.find('.front');
		if (front.attr('rel') != this.position.page) {
			el.addClass('front');
			this.$el.animate({ scrollLeft: el.offset().left - pages.position().left }, 400, function() { front.removeClass('front'); });
		}
	},
...
var AddressView = Backbone.View.extend({
	...
	initialize: function() {
		...
		_.each(quran_suras, function(item, i) {
			name = item;
			tokens = [name, name.replace('أ', 'ا').replace('إ', 'ا')];
			if (item.indexOf('ال') == 0) {
				name = name.substr(2);
				tokens.push(name); tokens.push(name.replace('أ', 'ا').replace('إ', 'ا'));
			}
